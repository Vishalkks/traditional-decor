...
def generate(BATCH_SIZE, nice=False):
    ...
    if nice:
        ...
        print(nice_images.shape)
        print(generated_images.shape)
        for i in range(BATCH_SIZE):
            idx = int(pre_with_index[i][1])
            nice_images[i, :, :, :] = generated_images[idx, :, :, :]
            img = Image.fromarray(nice_images[i], 'RGBA')
            img.save("images/generated_%d.png" % i)
            img.show()
...
def get_data():
    ...
    #resized_images = np.array(pickle.load(open("resized_images112.pkl", "rb")))
...
if __name__ == "__main__":
    ...
    generate(BATCH_SIZE=4, nice=True)